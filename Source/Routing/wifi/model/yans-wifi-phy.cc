...
YansWifiPhy::StartTx (Ptr<Packet> packet, WifiTxVector txVector, Time txDuration)
{
  ...
  NS_LOG_DEBUG("\tschedule TX Complete at " << txDuration << " \n" );
  Simulator::Schedule( (txDuration), &YansWifiPhy::TransmitComplete, this, packet );
}

void 
YansWifiPhy::TransmitComplete(Ptr<Packet const> packet) {
   NotifyTxEnd (packet);
}
