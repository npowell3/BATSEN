...
NS_LOG_COMPONENT_DEFINE ("BatmanWifi");
...
main (int argc, char *argv[])
{
   ...
   batmanNodes.Add (wifiStaNodes);
   batmanNodes.Add (wifiSrvrNode);
   ...
   positionAlloc->Add (Vector (50,75,0));
   positionAlloc->Add (Vector (150,25,0));
   positionAlloc->Add (Vector (145,30,0));
   ...
   waymobility.Install(wifiStaNodes.Get(3));
   // First we do the center station
   mob = wifiStaNodes.Get(3)->GetObject<WaypointMobilityModel>();
   Waypoint waySta1_p0 = Waypoint(Seconds(0),Vector(200,50,0));
   mob->AddWaypoint(waySta1_p0);
   Waypoint waySta1_p1 = Waypoint(Seconds(8),Vector(200,50,0));
   mob->AddWaypoint(waySta1_p1);
   Waypoint waySta1_p2 = Waypoint(Seconds(14),Vector(150,50,0));
   mob->AddWaypoint(waySta1_p2); 
   Waypoint waySta1_p3 = Waypoint(Seconds(28),Vector(150,50,0));
   mob->AddWaypoint(waySta1_p3); 
   Waypoint waySta1_p4 = Waypoint(Seconds(40),Vector(200,50,0));
   ...
   internet_olsr.Install (batmanNodes);
   ...
   AnimationInterface anim ("batman-mcomplex.xml"); // Mandatory
   ...
   anim.EnableIpv4RouteTracking ("batman-routingtable-wireless.xml", Seconds (0), Seconds (45), Seconds (0.25)); //Optional
   anim.EnableWifiMacCounters (Seconds (0), Seconds (10)); //Optional
   anim.EnableWifiPhyCounters (Seconds (0), Seconds (10)); //Optional
   anim.SetMaxPktsPerTraceFile(100000000);
   
   // Setup tracing in PCAP
   phy.EnablePcap ("batman-mcomplex-server", srvrDevice);
   phy.EnablePcap ("batman-mcomplex-stations", staDevices);
   ...
   const char *filename = "mcomplex-batmanoutput.csv\0";
