...
NS_LOG_COMPONENT_DEFINE ("BatmandWifi");
...
//  wifiApNode.Get (0)->AggregateObject (energySource);
  ...
  InternetStackHelper internet_batman;
  internet_batman.SetRoutingHelper (list); // has effect on the next Install ()
  internet_batman.Install (batmanNodes);
...
//  Ipv4GlobalRoutingHelper::PopulateRoutingTables ();
  ...
  AnimationInterface anim ("batman-animation.xml"); // Mandatory
  ...
  anim.EnableIpv4RouteTracking ("batman-wifi-routingtable-wireless.xml", Seconds (0), Seconds (maxSimulatedTime), Seconds (0.5));
  anim.EnableWifiMacCounters (Seconds (0), Seconds (10)); //Optional
  anim.EnableWifiPhyCounters (Seconds (0), Seconds (10)); //Optional
  anim.SetMaxPktsPerTraceFile(100000000);

  // Setup tracing in PCAP
  phy.EnablePcap ("batman-hna", staDevices, true);
  phy.EnablePcap ("batman-hna", srvrDevice, true);
...
*/
  ...
  const char *filename = "batman-3node.csv\0";
