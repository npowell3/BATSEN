...
NS_LOG_COMPONENT_DEFINE ("AodvBigBox");
...
//                                 "Speed", StringValue ("ns3::ConstantRandomVariable[Constant=12.0]"),
       ...
       */
      AodvHelper olsr;                          // must assign fixed random streams after installation
      ...
      NS_LOG_DEBUG(" assigned " << strmCnt << " rnd streams to Aodv protocol");
      strmTot += strmCnt;
      ...
      char animName[250];
      sprintf( animName, "aodv-big-box-animation-%d.xml", x);
      ...
      char filename[250];
      sprintf(filename, "aodv-data_%d.csv", x);
