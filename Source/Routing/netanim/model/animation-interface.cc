  ...
  if (!Simulator::IsFinished ())
    {
      ...
      PurgePendingPackets (AnimationInterface::ETHERNET);
...
void 
AnimationInterface::EthernetPhyTxBeginTrace (std::string context, Ptr<const Packet> p)
{
  NS_LOG_FUNCTION (this);
  CHECK_STARTED_INTIMEWINDOW_TRACKPACKETS;
  Ptr <NetDevice> ndev = GetNetDeviceFromContext (context);
  ...
  UpdatePosition (ndev);
  ++gAnimUid;
  NS_LOG_INFO ("EthernetPhyTxBeginTrace for packet:" << gAnimUid);
  AddByteTag (gAnimUid, p);
  UpdatePosition (ndev);
  AnimPacketInfo pktInfo (ndev, Simulator::Now ());
  AddPendingPacket (AnimationInterface::ETHERNET, gAnimUid, pktInfo);
}

void 
AnimationInterface::EthernetPhyTxEndTrace (std::string context, Ptr<const Packet> p)
{
  NS_LOG_FUNCTION (this);
  CHECK_STARTED_INTIMEWINDOW_TRACKPACKETS;
  Ptr <NetDevice> ndev = GetNetDeviceFromContext (context);
  ...
  UpdatePosition (ndev);
  uint64_t animUid = GetAnimUidFromPacket (p);
  NS_LOG_INFO ("EthernetPhyTxEndTrace for packet:" << animUid);
  if (!IsPacketPending (animUid, AnimationInterface::ETHERNET))
    {
      NS_LOG_WARN ("EthernetPhyTxEndTrace: unknown Uid"); 
      NS_FATAL_ERROR ("EthernetPhyTxEndTrace: unknown Uid"); 
      AnimPacketInfo pktInfo (ndev, Simulator::Now ());
      AddPendingPacket (AnimationInterface::ETHERNET, animUid, pktInfo);
      NS_LOG_WARN ("Unknown Uid, but adding Ethernet Packet anyway");
    }
  /// \todo NS_ASSERT (IsPacketPending (AnimUid) == true);
  AnimPacketInfo& pktInfo = m_pendingEthernetPackets[animUid];
  pktInfo.m_lbTx = Simulator::Now ().GetSeconds ();
}

void 
AnimationInterface::EthernetPhyRxEndTrace (std::string context, Ptr<const Packet> p)
{
  NS_LOG_FUNCTION (this);
  CHECK_STARTED_INTIMEWINDOW_TRACKPACKETS;
  Ptr <NetDevice> ndev = GetNetDeviceFromContext (context);
  ...
  if (!IsPacketPending (animUid, AnimationInterface::ETHERNET))
    {
      NS_LOG_WARN ("EthernetPhyRxEndTrace: unknown Uid"); 
      return;
    }
  /// \todo NS_ASSERT (EthernetPacketIsPending (AnimUid) == true);
  AnimPacketInfo& pktInfo = m_pendingEthernetPackets[animUid];
  pktInfo.ProcessRxBegin (ndev, Simulator::Now ().GetSeconds ());
  NS_LOG_INFO ("EthernetPhyRxEndTrace for packet:" << animUid);
  NS_LOG_INFO ("EthernetPhyRxEndTrace for packet:" << animUid << " complete");
  OutputEthernetPacket (p, pktInfo);
}

void 
AnimationInterface::EthernetMacRxTrace (std::string context, Ptr<const Packet> p)
{
  NS_LOG_FUNCTION (this);
  CHECK_STARTED_INTIMEWINDOW_TRACKPACKETS;
  Ptr <NetDevice> ndev = GetNetDeviceFromContext (context);
  ...
  if (!IsPacketPending (animUid, AnimationInterface::ETHERNET))
    {
      NS_LOG_WARN ("EthernetMacRxTrace: unknown Uid"); 
      return;
    }
  /// \todo NS_ASSERT (EthernetPacketIsPending (AnimUid) == true);
  AnimPacketInfo& pktInfo = m_pendingEthernetPackets[animUid];
...
void 
AnimationInterface::OutputEthernetPacket (Ptr<const Packet> p, AnimPacketInfo &pktInfo)
{
  CheckMaxPktsPerTraceFile ();
  NS_ASSERT (pktInfo.m_txnd);
  uint32_t nodeId = pktInfo.m_txnd->GetNode ()->GetId ();
  ...
  switch (protocolType)
    {
      ...
      case AnimationInterface::ETHERNET:
        {
          pendingPackets = &m_pendingEthernetPackets;
          break;
        }
...
AnimationInterface::ConnectCallbacks ()
{
  ...
  Config::Connect ("/NodeList/*/DeviceList/*/$ns3::EthernetNetDevice/PhyTxBegin",
                   MakeCallback (&AnimationInterface::EthernetPhyTxBeginTrace, this));
  Config::Connect ("/NodeList/*/DeviceList/*/$ns3::EthernetNetDevice/PhyTxEnd",
                   MakeCallback (&AnimationInterface::EthernetPhyTxEndTrace, this));
  Config::Connect ("/NodeList/*/DeviceList/*/$ns3::EthernetNetDevice/PhyRxEnd",
                   MakeCallback (&AnimationInterface::EthernetPhyRxEndTrace, this));
  Config::Connect ("/NodeList/*/DeviceList/*/$ns3::EthernetNetDevice/MacRx",
                   MakeCallback (&AnimationInterface::EthernetMacRxTrace, this));
  ...
  for (NodeList::Iterator i = NodeList::Begin (); i != NodeList::End (); ++i)
    {
      ...
      for (uint32_t i = 0; i < nDev; ++i)
 	  ...
 	  NS_ASSERT (dev);
          ...
          if ((channelType == std::string ("ns3::PointToPointChannel")) || (channelType == std::string ("ns3::EthernetChannel")))
