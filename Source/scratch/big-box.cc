...
NS_LOG_COMPONENT_DEFINE ("BigBox");
...
main (int argc, char *argv[])
{
  ...
  for ( int x = 0; x < nIter; ++x )
  {
    for ( uint8_t itr = 0; itr < 2; ++itr )
    {
      ...
      NS_LOG_DEBUG(" assigned " << strmCnt << " rnd streams to " << ((itr==0) ? "BATMAN" : "OLSR") << " protocol");
      strmTot += strmCnt;
      ...
      if ( itr == 0 )
        animation_filename = "batman-animation.xml";
      else
        animation_filename = "olsr-animation.xml";
      ...
      {
        phy.EnablePcap ("olsr-hna", staDevices, true);
      ...
      if ( itr == 0 )
      {
        sprintf( filename, "batman-bigbox-%02d.csv", j );
        aggregator->PlotData( filename, maxSimulatedTime, 1.0 );
      }
      else
      {
        sprintf( filename, "olsr-bigbox-%02d.csv", j );
        aggregator->PlotData( filename, maxSimulatedTime, 1.0 );
      }
