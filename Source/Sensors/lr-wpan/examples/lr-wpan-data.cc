...
int main (int argc, char *argv[])
{
  ...
  // Animation support
  AnimationInterface anim ("lrwpan-data-animation.xml"); // Mandatory
  anim.SetConstantPosition( n0, 5.0, 5.0, 0.0 );
  anim.SetConstantPosition( n1, 10.0, 10.0, 0.0 );
  anim.EnablePacketMetadata (); // Optional
  anim.SetMaxPktsPerTraceFile(100000000);
  anim.SetMobilityPollInterval( Seconds( 0.1 ) ); 
  anim.UpdateNodeDescription( n0, "Sender" );
  anim.UpdateNodeDescription( n1, "Receiver" );
  anim.UpdateNodeSize( n0->GetId(), 0.1, 0.1 );
  anim.UpdateNodeSize( n1->GetId(), 0.1, 0.1 );
