...
typedef enum
{
  ...
  IEEE_802_15_4_PHY_TX_FAIL = 0xd      // special TX fail due to RX interruption
} LrWpanPhyEnumeration;
  
const char g_phy_string[IEEE_802_15_4_PHY_TX_FAIL+1][20] = {
  {"BUSY"},
  {"BUSY RX"},
  {"BUSY TX"},
  {"FRC TRX OFF"},
...
//  uint8_t phyTransmitPower;          //!< Transmit power
  int16_t phyTransmitPower;          //!< Transmit power
...
public:
  ...
  void SetAddr( Mac16Address addr ) { m_addr = addr; };
...
private:
  ...
  /**
   * New local member variable to record the most recent SINR from the
   * active reception.  Needed to support unique SINR calculations per
   * device since the packet is a Ptr<Packet>, and is the same for all
   * nodes in the net.
   */
  double m_sinr;
