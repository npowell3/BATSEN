...
LrWpanPhy::StartRx (Ptr<SpectrumSignalParameters> spectrumRxParams)
{
  ...
  NS_LOG_DEBUG( "StartRx \t FSM " << g_phy_string[m_trxState] );
  LrWpanSpectrumValueHelper psdHelper;
  ...
  if (m_trxState == IEEE_802_15_4_PHY_RX_ON && !m_setTRXState.IsRunning ())
  {
    ...
    // SINR.
    NS_LOG_DEBUG (this << " receiving packet with power: " << 10 * log10(LrWpanSpectrumValueHelper::TotalAvgPower (lrWpanRxParams->psd, m_phyPIBAttributes.phyCurrentChannel)) + 30 << "dBm @ " << Now() );
    ...
    *interferenceAndNoise += *m_noise;
    ...
    NS_LOG_DEBUG("\t sinr " << sinr << " where [10 * log10(sinr)] = " << (10 * log10 (sinr)) );
    ...
    if (10 * log10 (sinr) > -5)
    {
      ...
      NS_LOG_DEBUG( this << " making currentRxPacket pair with " << lrWpanRxParams );
    ...
    {
      NS_LOG_DEBUG( this << " drop packet at Start Rx " );
      m_phyRxDropTrace (p);
    }
  }
...
LrWpanPhy::CheckInterference (void)
{
  ...
  if (m_trxState == IEEE_802_15_4_PHY_BUSY_RX)
  {
    ...
    {
      NS_LOG_DEBUG( "Node " << m_addr << " w curRxParam " << currentRxParams << " sec " << m_currentRxPacket.second );
    }
    else
    {
      ...
      if (m_errorModel != 0)
      {
        ...
        *interferenceAndNoise += *m_noise;
        ...
        NS_LOG_DEBUG("  Check Interference: ms " << t << "chunck " << chunkSize << " sinr " << sinr << " per " << per );
        ...
        uint8_t lqi = tag.Get ();
        NS_LOG_DEBUG("    pre mod LQI " << (int) lqi );
        tag.Set (lqi - (per * lqi));
        NS_LOG_DEBUG("    calcd LQI " << (per * lqi));
        currentPacket->ReplacePacketTag (tag);
        
        m_sinr = sinr;
        
        //          LrWpanSinrTag stag( sinr );
        //          currentPacket->AddPacketTag( stag );
    ...
    NS_LOG_DEBUG( "[address " << m_addr << "] " << this << " CheckInterference in state " << g_phy_string[m_trxState] );
...
LrWpanPhy::EndRx (Ptr<SpectrumSignalParameters> par)
{
  ...
  NS_LOG_DEBUG( this << " Node " << m_addr << " EndRx @ " << Now() << " when state is: " << g_phy_string[m_trxState] << " pending state " << g_phy_string[m_trxStatePending] );

  if ( ( m_trxState != IEEE_802_15_4_PHY_RX_ON ) &&
       ( m_trxState != IEEE_802_15_4_PHY_BUSY_RX ) )
  {
    NS_LOG_DEBUG("    hit EndRx when state is " << g_phy_string[m_trxStatePending] );
    m_signal->RemoveSignal (par->psd);
    return;
  }
  else
    NS_LOG_DEBUG("    working the EndRx");
  
  Ptr<LrWpanSpectrumSignalParameters> params = DynamicCast<LrWpanSpectrumSignalParameters> (par);

  if (!m_edRequest.IsExpired ())
  {
    NS_LOG_UNCOND( " end rx - m_edRequest is not expired" );
    // Update the average receive power during ED.
    Time now = Simulator::Now ();
    m_edPower.averagePower += LrWpanSpectrumValueHelper::TotalAvgPower (m_signal->GetSignalPsd (), m_phyPIBAttributes.phyCurrentChannel) * (now - m_edPower.lastUpdate).GetTimeStep () / m_edPower.measurementLength.GetTimeStep ();
    m_edPower.lastUpdate = now;
  ...
  {
    NS_LOG_DEBUG("\tcalling CheckInterference");
    CheckInterference ();
  }

  // Update the interference.
  NS_LOG_DEBUG( "Node: " << m_addr << " remove signal " << par->psd << " from SpctrmSigParm " << par );
  m_signal->RemoveSignal (par->psd);

  if (params == 0)
  {
    NS_LOG_DEBUG ("Node: " << m_device->GetAddress() << " Removing interferent: " << *(par->psd));
    return;
  }
  ...
  if (currentRxParams == params)
  {
    ...
    // Make a copy of the current packet to avoid modifying the pointer to
    // the same packet shared across multiple devices
    Ptr<Packet> nPkt = currentPacket->Copy();
    
    NS_LOG_DEBUG( "\torig pkt " << currentPacket << " new pkt " << nPkt << " w/SNR " << m_sinr );
    NS_ASSERT( nPkt != currentPacket );
    
    LrWpanSinrTag stag( m_sinr );
    nPkt->AddPacketTag( stag );
    ...
    if (!m_currentRxPacket.second)
    {
      ...
      {
        NS_LOG_DEBUG (this << "   packet RX'ed with power: " << 10 * log10(LrWpanSpectrumValueHelper::TotalAvgPower (params->psd, m_phyPIBAttributes.phyCurrentChannel)) + 30 << "dBm @ " << Now() );

        m_pdDataIndicationCallback (nPkt->GetSize (), nPkt, tag.Get ());
      }
    }
    else
    {
    ...
    {
      NS_LOG_WARN( " m_trxStatePending " << g_phy_string[m_trxStatePending] << " is not IDLE " );
      ChangeTrxState (IEEE_802_15_4_PHY_RX_ON);
    }
  }
  else
    NS_LOG_WARN(" currentRxParams != params " << currentRxParams << " " << params );
  
  NS_LOG_DEBUG("\t end of EndRx");
}
...
LrWpanPhy::PdDataRequest (const uint32_t psduLength, Ptr<Packet> p)
{
  ...
  //NS_LOG_UNCOND( "  Phy TX start for node " << m_addr << " state " << g_phy_string[m_trxStatePending] << " @ " << Now() );

  if (psduLength > aMaxPhyPacketSize)
  {
    NS_LOG_UNCOND ("Drop packet because psduLength too long:\n\t length:" << psduLength << "\n\t max size: " << aMaxPhyPacketSize );
  ...
  if (!m_setTRXState.IsRunning ())
      ...
      if (m_trxState == IEEE_802_15_4_PHY_TX_ON)
        {
          ...
          NS_LOG_WARN(" \t schd end TX in " << txParams->duration << " or at " << Now() + txParams->duration );
          
          ChangeTrxState (IEEE_802_15_4_PHY_BUSY_TX);
          
          NS_LOG_WARN("   Node " << m_addr << " changed PHY state to " << g_phy_string[m_trxState] );
  ...
  else
    {
      ...
      //NS_LOG_UNCOND ("Drop packet because trcvr state !running ? " << m_setTRXState.IsRunning() );
      if (!m_pdDataConfirmCallback.IsNull ())
        {
          m_pdDataConfirmCallback (IEEE_802_15_4_PHY_UNSPECIFIED);
        }
...
LrWpanPhy::PlmeSetTRXStateRequest (LrWpanPhyEnumeration state)
{
  ...
  NS_LOG_LOGIC ("Trying to set m_trxState from " << g_phy_string[m_trxState] << " to " << g_phy_string[state] );
...
{
  NS_LOG_LOGIC (this << " state: " << g_phy_string[m_trxState] << " -> " << g_phy_string[newState] );
...
{
  bool needToAbort = ( (m_trxState != IEEE_802_15_4_PHY_BUSY_TX) && (m_trxState != IEEE_802_15_4_PHY_TRX_OFF) );
  
  NS_LOG_FUNCTION (this);
  
  if ( needToAbort )
    NS_LOG_INFO("\tm_trxState is " << g_phy_string[m_trxState] << " when ending TX @ " << Now() << " MAC " << m_addr );

  //NS_ABORT_IF ( (m_trxState != IEEE_802_15_4_PHY_BUSY_TX) && (m_trxState != IEEE_802_15_4_PHY_TRX_OFF) );

  if (m_currentTxPacket.second == false)
    {
      NS_LOG_INFO ("Packet successfully transmitted");
    ...
    {
      NS_LOG_INFO ("Packet transmission aborted");
  ...
  if (m_trxStatePending != IEEE_802_15_4_PHY_IDLE)
    {
      NS_LOG_INFO("\tpending state " << g_phy_string[m_trxStatePending] );
      // Only change the state immediately, if the transceiver is not already
      // switching the state.
      if (!m_setTRXState.IsRunning ())
        {
    ...
    {
      NS_LOG_INFO("\tcheck if we need to turn on TX ");
      if (m_trxState != IEEE_802_15_4_PHY_TRX_OFF)
        {
          if ( needToAbort )
            ChangeTrxState (IEEE_802_15_4_PHY_TX_FAIL);
          else
            ChangeTrxState (IEEE_802_15_4_PHY_TX_ON);
        }
    }
